...
export default function FinancialDashboard() {
  ...
  useEffect(() => {
    if (user) {
      fetch("http://localhost:8080/transferencias", {
        ...
        .then((data) => {
          setVendasPdv(data.trasferencias.filter(t => t.loja == user.Loja_vinculada));
        });
      ...
      fetch("http://localhost:8080/contas", {
        ...
        .then((data) => {
          setContas(data.contas.filter(c => c.loja == user.Loja_vinculada));
        });
      fetch("http://localhost:8080/despesas", {
        ...
        .then((data) => {
          setDespesas(data.despesas.filter(d => d.loja == user.Loja_vinculada));
        });
      fetch("http://localhost:8080/produtos", {
        ...
        .then((data) => {
          setProdutos(data.produtos.filter(p => parseInt(p.filial) == user.Loja_vinculada));
        })
    }
  }, [user]);
  //  LÓGICA DE NEGÓCIO
  ...
  // useEffect(() => {
  //   if (filialFilter) {
  //     return fluxo.filter(i => i.filial == filialFilter).filter(i => {
  //       const diff = (new Date() - new Date(i.horario)) / (86400000)
  //       periodo === 'diario' ? diff <= 1 : periodo === 'semanal' ? diff <= 7 : diff <= 30
  ...
  useEffect(() => {
    if (user) {
      setFilialFilter(user.Loja_vinculada)
    }
  }, [user]);
  ...
  const exportPDF = (filial) => {
    const receitaFilial = vendasPdv
      ? vendasPdv
        .filter(v => v.id == filial.id)
        .reduce((sum, f) => sum + f.preco * f.quantidade_produto, 0)
      : 0;
    const despesasFilial = despesas
      ? despesas
        .filter(d => d.loja == filial.id)
        .reduce((sum, f) => sum + f.valor, 0)
      : 0;

    const doc = new jsPDF(); doc.text(`Relatório ${filial.nome}`, 14, 20)
    autoTable(doc, { startY: 30, head: [["Métrica", "Valor"]], body: [["Receita", formatCurrency(receitaFilial)], ["Saldo", formatCurrency(receitaFilial - despesasFilial)]] })
