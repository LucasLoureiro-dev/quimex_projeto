...
// --- COMPONENTES UTILITÁRIOS PARA O MISTURADOR ---
const hexToRgb = (hex) => {
...
// --- COMPONENTE MISTURADOR DE TINTA (MODAL) ---
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 bg-gray-50 p-8 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-gray-200">
          ...
          <div className="relative group">
            ...
            <svg width="240" height="240" viewBox="0 0 200 220" className="drop-shadow-xl transition-transform duration-300 hover:scale-105">
              <defs>
              ...
              <ellipse cx="100" cy="60" rx="70" ry="16" fill={color} className="transition-colors duration-300" />
              
              <path d="M 21 90 C 21 90, 100 110, 179 90 L 179 150 C 179 150, 100 170, 21 150 Z" fill="#117540" />
              <g transform="translate(45, 100)">
                ...
                <text x="50" y="28" fontFamily="sans-serif" fontWeight="bold" fontSize="24" fill="white">Quimex</text>
              </g>
          ...
          <p className="mt-6 text-sm text-gray-500 text-center px-4">
            A cor selecionada será preparada automaticamente pelo sistema tintométrico Quimex.
          </p>
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col">
          <div className="flex justify-between items-start mb-6">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Configurar Cor</h2>
              <p className="text-sm text-gray-500">Escolha a cor para o lote de tinta.</p>
            </div>
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex gap-4 items-center p-4 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <div className="flex-1">
                <label className="text-xs font-bold text-gray-500 uppercase">Código Hex</label>
                <input 
                  type="text" 
              ...
              {['r', 'g', 'b'].map((channel) => (
                <div key={channel} className="space-y-1">
                  <div className="flex justify-between text-xs font-semibold uppercase text-gray-500">
                    <span>{channel === 'r' ? 'Vermelho' : channel === 'g' ? 'Verde' : 'Azul'}</span>
                  ...
                  <input 
                    ...
                    className={`w-full h-2 rounded-lg appearance-none cursor-pointer 
                      ${channel === 'r' ? 'bg-red-100 accent-red-500' : 
                        channel === 'g' ? 'bg-green-100 accent-green-500' : 
...
const categories = ["Todos", "Tintas", "Ácidos", "Bases", "Solventes", "Sais", "Oxidantes"];
...
export default function QuimexPOS() {
  ...
  // Verificação básica de sessão (mantida do seu código)
  useEffect(() => {
    // Apenas executa no client-side para evitar erro de SSR
    if (typeof window !== 'undefined') {
        const turnoId = sessionStorage.getItem("turnoId");
        // if (!turnoId) router.push("/loginVendedor"); // Comentado para visualização
    }
  }, []);
        ...
        <header className="border-b border-gray-200 bg-white px-6 py-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-800 tracking-tight">QUIMEX <span className="text-green-600">POS</span></h1>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right hidden md:block">
                <p className="text-xs text-slate-400 font-semibold uppercase">Operador</p>
                <div className="flex items-center gap-2">
                   <div className="h-2 w-2 rounded-full bg-green-500 animate-pulse"></div>
                   <p className="text-sm font-bold text-slate-700">
                     {typeof window !== 'undefined' ? (localStorage.getItem("usuarioLogado") || "Carlos Silva") : "Usuario"}
                   </p>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-3">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-4 opacity-60">
              ...
              <p className="text-sm font-medium">Caixa livre. Aguardando produtos.</p>
            </div>
          ) : (
            cart.map((item) => (
              <div key={item.id} className="bg-white border border-gray-100 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow group relative">
                <div className="flex gap-3">
                  ...
                  <div className="w-16 h-16 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {item.selectedColor ? (
                      ...
                      <div className="w-full h-full relative group-hover:scale-110 transition-transform">
                          <div className="absolute inset-0" style={{ backgroundColor: item.selectedColor }}></div>
                      ...
                      <FlaskConical className="text-gray-300 h-8 w-8" />
                    )}
                  </div>

                  <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                    <div>
                        <h3 className="font-semibold text-sm text-slate-800 leading-tight truncate" title={item.name}>
                            {item.name}
                        </h3>
        ...
        <div className="border-t border-gray-200 p-5 bg-white space-y-4">
          <div className="space-y-2">
             <div className="flex justify-between items-center text-sm text-gray-500">
                <span>Subtotal ({cart.reduce((acc, i) => acc + i.quantity, 0)} itens)</span>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-8 flex flex-col items-center">
              <p className="text-sm text-gray-500 mb-1 font-medium uppercase tracking-wide">Valor a Receber</p>
              <p className="text-5xl font-bold text-slate-800 mb-8">R$ {total.toFixed(2)}</p>

              <div className="grid grid-cols-2 gap-4 w-full">
                {[
                    { id: 'Débito', icon: CreditCard, color: 'text-blue-600', bg: 'bg-blue-50 hover:bg-blue-100 hover:border-blue-200' },
                    { id: 'Crédito', icon: CreditCard, color: 'text-purple-600', bg: 'bg-purple-50 hover:bg-purple-100 hover:border-purple-200' },
                    { id: 'Dinheiro', icon: Banknote, color: 'text-green-600', bg: 'bg-green-50 hover:bg-green-100 hover:border-green-200' },
                    { id: 'PIX', icon: Smartphone, color: 'text-teal-600', bg: 'bg-teal-50 hover:bg-teal-100 hover:border-teal-200' },
                ].map((method) => (
                    <button
                        key={method.id}
                    ...
                    >
                        ...
                        <span className="font-semibold text-gray-700 text-sm">{method.id}</span>
                    </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 text-center border-t border-gray-100">
                <p className="text-xs text-gray-400">Transação segura e criptografada</p>
            </div>
          </div>
        </div>
      )}
