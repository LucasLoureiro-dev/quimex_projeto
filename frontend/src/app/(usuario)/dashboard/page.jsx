...
export default function DashboardPage() {
  ...
  const buscaProdutos = async () => {
    ...
    const produtos = await resJson.produtos
  ...
  const buscaFornecedores = async () => {
    ...
    const fornecedores = await resJson.fornecedores

    setFornecedores(fornecedores)
  }

  const buscaDespesas = async () => {
    const res = await fetch("http://localhost:8080/despesas")

    const resJson = await res.json()

    const despesas = await resJson.despesas
  ...
  const buscaContas = async () => {
    ...
    const contas = await resJson.contas
  ...
  const buscaLojas = async () => {
    ...
    const lojas = await resJson.lojas
  ...
  var contaPagos = 0;
  var contaPendentes = 0;
  var contaVencidos = 0;

  if (contas) {
    contas.map((i, k) => {
      if (i.estado == "pago") {
        contaPagos = contaPagos + 1
      }
      else if (i.estado == "pendente") {
        contaPendentes = contaPendentes + 1
      }
      else if (i.estado == "vencido") {
        contaVencidos = contaVencidos + 1
      }
    })
  }

  const invoiceData = [
    { name: "Pagos", value: contaPagos },
    { name: "Pendentes", value: contaPendentes },
    { name: "Vencidos", value: contaVencidos },
  ]
    ...
    <main className="container mx-auto px-4 py-8 max-w-7xl bg-background text-foreground transition-colors duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        {[
          ...
          {
            title: "Lucro do Mês",
            value: `R$ ${despesas.lucro ? despesas.lucro : 0}`,
            icon: ShoppingCart,
          },
          {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
        {[
          { title: "Total de Funcionários", value: funcionarios.length, icon: Users },
          { title: "Total de Lojas", value: lojas.length, icon: Building2 },
        ].map(({ title, value, icon: Icon }) => (
          <div
            key={title}
